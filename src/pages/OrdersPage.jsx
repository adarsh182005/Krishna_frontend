...
const OrdersPage = () => {
    ...
    const fetchOrders = async () => {
      try {
        ...
        const response = await axios.get(
          `${backendUrl}/api/orders/myorders`, // Use the correct "myorders" route
          {
            headers: {
        ...
        setError('Failed to load your orders. Please try logging in again.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const getStatusColor = (isDelivered) => {
    return isDelivered ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800';
  };
  ...
  if (error) {
    return (
      <div className="min-h-screen flex items-center justify-center text-center">
        <div>
            <h1 className="text-xl font-bold text-red-600 mb-2">Could Not Fetch Orders</h1>
              ...
              <div key={order._id} className="bg-white rounded-lg shadow-lg p-6">
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-4">
                  ...
                  <div className="flex flex-col sm:flex-row gap-2">
                    <span className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(order.isDelivered)}`}>
                      {order.isDelivered ? 'DELIVERED' : 'PENDING'}
                    </span>
                    <span className={`px-3 py-1 rounded-full text-xs font-medium ${getPaymentStatusColor(order.isPaid)}`}>
                ...
                <div className="mt-6 flex flex-col sm:flex-row gap-3">
                  <button
                    // *** FIX: This now points to the correct order details page ***
